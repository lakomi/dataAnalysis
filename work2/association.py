...
class Association(object):
    ...
    def create_C1(self, dataset):
        # 遍历dataset，构建全部可能的单元素候选项集合(list)
        # 每个单元素候选项：（属性名，属性取值）
        C1 = []
        for data in dataset:
            for item in data:
            ...
            for freq_set in L[i]:
                ...
                if i > 1:
                    self.rules_from_conseq(
                        freq_set, H1, support_data, big_rules_list)
                else:
    ...
    def rules_from_conseq(self, freq_set, H, support_data, big_rules_list):
        ...
        if len(freq_set) > (m+1):
            ...
            if len(Hmp1) > 1:
                self.rules_from_conseq(
                    freq_set, Hmp1, support_data, big_rules_list)
    ...
    def cal_conf(self, freq_set, H, support_data, big_rules_list):
        ...
        for conseq in H:
            ...
            if conf >= self.min_confidence:
                big_rules_list.append(
                    (freq_set-conseq, conseq, sup, conf, lift))
